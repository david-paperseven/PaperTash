...
@synthesize originalView = _originalView, labelWidth = _labelWidth, showNetworkActivityIndicator = _showNetworkActivityIndicator,
			activityViewWrapper = _activityViewWrapper;
...
- (id)initWithLabel:(NSString*)labelText width:(NSUInteger)labelWidth;
{
	...
	// we need a full screen view controller to hold the view
	_activityViewWrapper = [[P31RotatingViewController alloc] init];	
	[[UIApplication sharedApplication].keyWindow addSubview:_activityViewWrapper.view];
	
    // Allow subclasses to change the view to which to add the activity view (e.g. to cover the keyboard):
    self.originalView = _activityViewWrapper.view;
...
- (void)orientationDidChangeNotification:(NSNotification*)note
{
	...
	_activityViewWrapper.view.transform = TRotateTransformForOrientation( [UIApplication sharedApplication].statusBarOrientation );
	_activityViewWrapper.view.frame = [UIScreen mainScreen].bounds;
...
+ (void)removeView;
{
	...
	[activityView.activityViewWrapper.view removeFromSuperview];
	activityView.activityViewWrapper = nil;
...
- (id)initWithLabel:(NSString*)labelText width:(NSUInteger)labelWidth withImage:(UIImage*)image
{
	if( self = [super initWithLabel:labelText width:labelWidth] )
	{
		...
		_activityIndicator = [[UIImageView alloc] initWithImage:image];
	}
	return self;
}
