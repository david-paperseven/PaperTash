...
function GetTotalSplineLength(catPoints : Array)
{
    ...
    {
        point = catPoints[i];
        length += (point.pos-pos).magnitude;
        pos = point.pos;
    }
    return length;
}
...
function ConstructMesh(catPoints : Array)
{
    ...
    // start slim
    point = catPoints[0];
    AddPoint(m,point.pos);
    for (var i = 1; i < catPoints.Count-1; i++)
    {
        point = catPoints[i];

        var prevPoint : CatPoint = catPoints[i-1];
        direction = point.pos - prevPoint.pos;
        ...
        AddPoint(m,point.pos + up*width*tashWidth + Camera.main.transform.forward*tashHeight);
        AddPoint(m,point.pos - up*width*tashWidth + Camera.main.transform.forward*tashHeight);
...
function DrawWireframe()
{
    ...
    for (index = 0; index < gMeshes.Count; index++)
    {
        ...
        for (var i=0;i<numTriangles;i++)
        {
            ...
            var vertB : Vert = m.verts[tri.iB];
            var vertC : Vert = m.verts[tri.iC];
            var p1 : Vector3  = vertA.pos-Camera.main.transform.forward*0.003;
            var p2 : Vector3  = vertB.pos-Camera.main.transform.forward*0.003;
            var p3 : Vector3  = vertC.pos-Camera.main.transform.forward*0.003;
